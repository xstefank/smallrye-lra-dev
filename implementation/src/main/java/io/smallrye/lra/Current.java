...
public class Current {
    ...
    public static void updateLRAContext(ContainerResponseContext responseContext) {
        ...
        if (lraId != null) {
            responseContext.getHeaders().putSingle(LRAClient.LRA_HTTP_HEADER, lraId);
        } else {
            responseContext.getHeaders().remove(LRAClient.LRA_HTTP_HEADER);
        }
    }

    public static void updateLRAContext(URL lraId, MultivaluedMap<String, String> headers) {
        headers.putSingle(LRAClient.LRA_HTTP_HEADER, lraId.toString());
        push(lraId);
    }
    ...
    public static void updateLRAContext(ClientRequestContext context) {
        ...
        if (lraId != null) {
            context.getHeaders().putSingle(LRAClient.LRA_HTTP_HEADER, lraId);
        } else {
            Object lraContext = context.getProperty(LRAClient.LRA_HTTP_HEADER);

            if (lraContext != null) {
                context.getHeaders().putSingle(LRAClient.LRA_HTTP_HEADER, lraContext);
            } else {
                context.getHeaders().remove(LRAClient.LRA_HTTP_HEADER);
            }
        }
    }
    ...
    public static void clearContext(MultivaluedMap<String, String> headers) {
        headers.remove(LRAClient.LRA_HTTP_HEADER);
        popAll();
    }
}
